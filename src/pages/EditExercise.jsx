...
export default function EditExercise() {
  ...
  useEffect(() => {
    Promise.all([api.getExercises(), api.getCategories(), api.getMuscleGroups()])
      .then(([exercises, cats, mgs]) => {
        ...
        setSelectedMuscleGroups(ex.muscle_groups.map((m) => m.id))
        setCategories(cats)
        setMuscleGroups(mgs)
      })
  ...
  const toggleMuscleGroup = (mgId) => {
    setSelectedMuscleGroups((prev) =>
      prev.includes(mgId) ? prev.filter((x) => x !== mgId) : [...prev, mgId]
    )
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.error ?? 'Failed to save changes. Please try again.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (error && !form) return <Layout><p className="text-sm text-red-600">{error}</p></Layout>
    ...
    <Layout>
      <div className="max-w-xl">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">Edit exercise</h1>
        </div>
